...
for m in range(1,step+1):
    ...
    coeff=[[column for column in range(n)] for row in range(n)]
    for i in range(0,n):
        for j in range(0,n):
            display[i][j]=""
            display2[i][j]=""
    ...
    DF=pd.DataFrame(coeff_np)
    DF.to_csv(f"coeff{m}.csv")
