import numpy as np
import pandas as pd
def oper(m,ini_dictionary1,n,ini_dictionary2):
    final_dictionary = {}
    for key in ini_dictionary1:
        final_dictionary[key] = m*ini_dictionary1[key] + n*ini_dictionary2.get(key, 0)
    for key in ini_dictionary2:
        if key not in final_dictionary:
            final_dictionary[key] = n*ini_dictionary2[key]
...
for m in range(1,step+1):
    ...
    for i in range(0,n):
        for j in range(0,n):
            if i-1>=0 and j-1>=0 and i+1<n and j+1<n:
               b[i][j]=oper(1,a[i][j],1,a[i][j+1])
               b[i][j]=oper(1,b[i][j],1,a[i+1][j+1])
               #b[i][j]=oper(1,b[i][j],1,a[i+1][j+1])
               #b[i][j]=oper(b[i][j],a[i+1][j+1])
    ...
    for i in range(0,n):
        for j in range(0,n):
            ...
            for key in a[i][j]:
                if bit!=0:
                        display[i][j]+=" +8 "+str(a[i][j][key])+str(key)
                else:
                ...
                if a[i][j][key]%8==0:
                    ...
                    if it!=0:
                        display2[i][j]+=" +8 "+str(a[i][j][key])+str(key)
                    else:
